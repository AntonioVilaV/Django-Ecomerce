...
class ComprarProductoCreateView(LoginRequiredMixin,validarGrupo,CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(ComprarProductoCreateView,self).dispatch(request, *args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        if "quant[1]" in self.request.POST:
            ...
            if cant > int(pro.inventario_producto.cantidad):
                return self.render_to_response(self.get_context_data(cantidad_error="Sin stock, ingrese una cantidad menor"))# especificar que debe pedir una cantidad menor de productos
        ...
        with transaction.atomic():
            ...
            if pro.get_descuento():
                ...
                descuento = (int(pro.precio)*cant)*(int(pro.descuento.descuento)/100)
...
class MisComprasActivasListView(LoginRequiredMixin,validarGrupo,ListView):
    ...
    def get_queryset(self):
        return RegistroVenta.objects.filter(usuario=self.request.user.pk).filter(state=True)
...
class MisComprasCerradasListView(LoginRequiredMixin,validarGrupo,ListView):
    ...
    def get_queryset(self):
        return RegistroVenta.objects.filter(usuario=self.request.user.pk).filter(state=False)
...
class DetalleCompraDetailView(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            venta = RegistroVenta.objects.filter(usuario=self.request.user).get(id=self.kwargs['pk'])
...
class CrearDatosEnvioCreateView(LoginRequiredMixin,CreateView): #SemiValidado - Recordar decidir cedula
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if logeado.is_authenticated:
            if RegistroVenta.objects.filter(id=self.kwargs['pk']).filter(usuario=logeado).exists() or RegistroVenta.objects.filter(id=self.kwargs['pk']).filter(producto__autor=logeado).exists():
...
class CrearDatosPagoCreateView(LoginRequiredMixin,CreateView):#Validado
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            RegistroVenta.objects.filter(id=self.kwargs['pk']).update(estado_operacion=estado_nuevo)
            if self.request.user.groups.get().name == 'Vendedor':
                return redirect("/detalleVenta/" + str(miVenta.id))
            elif self.request.user.groups.get().name == 'Comprador':
                return redirect("/detalleCompra/" + str(miVenta.id))
...
class MisVentasActivasListView(LoginRequiredMixin,validarGrupo,ListView):
    ...
    def get_queryset(self):
        return RegistroVenta.objects.filter(producto__autor__pk=self.request.user.pk).filter(state=True)
...
class MisVentasCerradasListView(LoginRequiredMixin,validarGrupo,ListView):
    ...
    def get_queryset(self):
        return RegistroVenta.objects.filter(producto__autor__pk=self.request.user.pk).filter(state=False)
...
class DetalleVentaUpdateView(LoginRequiredMixin,validarGrupo,UpdateView):
    ...
    def get_object(self, queryset=None):
        return RegistroVenta.objects.filter(producto__autor=self.request.user).filter(state=True).get(id=self.kwargs['pk'])
    ...
    def form_valid(self, form):
        if form.instance.estado_operacion.nombre == "Entregado" or form.instance.estado_operacion.nombre == "Cancelado":
            form.instance.state=False
        return super().form_valid(form)

class DetalleVentaCerradaTemplateView(LoginRequiredMixin,validarGrupo,TemplateView):#Validado
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            venta = RegistroVenta.objects.filter(producto__autor=self.request.user).get(id=self.kwargs['pk'])
            if not venta.state:
                return super().dispatch(request, *args, **kwargs)
            else:
