...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        created_categories = Categoria.objects.bulk_create([Categoria(nombre=category) for category in new_categories])
        for category in created_categories:
            self.stdout.write(self.style.SUCCESS('Category "%s" created' % category.nombre))
